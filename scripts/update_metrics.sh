...
# Count mistakes by category (use tr -d to remove newlines and spaces)
MISSING_TESTS=$(grep -c "^\*\*Category:\*\* missing-tests$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
MISSING_EXPOSURE=$(grep -c "^\*\*Category:\*\* missing-exposure$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
VIOLATED_TDD=$(grep -c "^\*\*Category:\*\* violated-tdd$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
VIOLATED_ARCH=$(grep -c "^\*\*Category:\*\* violated-architecture$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
MISSING_DOCS=$(grep -c "^\*\*Category:\*\* missing-docs$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
COMPLEXITY=$(grep -c "^\*\*Category:\*\* complexity-spike$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
OTHER=$(grep -c "^\*\*Category:\*\* other$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")

# Count by severity (use tr -d to remove newlines and spaces)
CRITICAL=$(grep -c "^\*\*Severity:\*\* critical$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
HIGH=$(grep -c "^\*\*Severity:\*\* high$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
MEDIUM=$(grep -c "^\*\*Severity:\*\* medium$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
LOW=$(grep -c "^\*\*Severity:\*\* low$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")

# Count by status (use tr -d to remove newlines and spaces)
OPEN=$(grep -c "^\*\*Status:\*\* open$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
FIXING=$(grep -c "^\*\*Status:\*\* fixing$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
PREVENTION_PENDING=$(grep -c "^\*\*Status:\*\* prevention-pending$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
MONITORING=$(grep -c "^\*\*Status:\*\* monitoring$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
RESOLVED=$(grep -c "^\*\*Status:\*\* resolved$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
ARCHIVED=$(grep -c "^\*\*Status:\*\* archived$" "$MISTAKES_FILE" 2>/dev/null | tr -d '\n ' || echo "0")
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -i '' "s/^- Total Mistakes: [0-9]*/- Total Mistakes: $TOTAL/" "$MISTAKES_FILE"
    sed -i '' "s/^- By Category: .*/- By Category: missing-docs ($MISSING_DOCS), missing-tests ($MISSING_TESTS), other ($OTHER)/" "$MISTAKES_FILE"
    sed -i '' "s/^- By Severity: .*/- By Severity: critical ($CRITICAL), high ($HIGH), medium ($MEDIUM)/" "$MISTAKES_FILE"
else
    sed -i "s/^- Total Mistakes: [0-9]*/- Total Mistakes: $TOTAL/" "$MISTAKES_FILE"
    sed -i "s/^- By Category: .*/- By Category: missing-docs ($MISSING_DOCS), missing-tests ($MISSING_TESTS), other ($OTHER)/" "$MISTAKES_FILE"
    sed -i "s/^- By Severity: .*/- By Severity: critical ($CRITICAL), high ($HIGH), medium ($MEDIUM)/" "$MISTAKES_FILE"
