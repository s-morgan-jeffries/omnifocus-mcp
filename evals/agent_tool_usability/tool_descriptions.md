...
Note: Completion status is NOT inherited. When a project is marked done or dropped, its incomplete tasks remain `completed: false` in the data. OmniFocus hides them in the UI, but get_tasks will return them unless you also check the project's status. To find truly actionable tasks, use `available_only=True` or check that the containing project is active.
